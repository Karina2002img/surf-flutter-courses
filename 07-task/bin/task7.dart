...
void main() {
  ...
  final resultMap = {};
  void sortingAndStructuring() {
    for (var item in productList) {
      if (item.qty > 0 && item.expirationDate.isAfter(DateTime.now())) {
        final categoryName = item.categoryName;
        final subcategoryName = item.subcategoryName;
        final productName = item.name;

        if (!resultMap.containsKey(categoryName)) {
          resultMap[categoryName] = {};
        }

        final categoryMap = resultMap[categoryName];
